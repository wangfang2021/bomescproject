    ...
    public class FP0009
    {
            ...
            try
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    {//有处理完成的数据
                        ...
                        sqlNQC.Append("where Process_Factory='" + vcPlant + "' and Process_YYYYMM='" + vcCLYM + "' and Process_Cycle_NO=" + iTimes + "    \n");
                        DataTable dtNQC = NQCSearch(sqlNQC.ToString());
                        if (dtNQC.Rows.Count > 0)
                        {
                            dtNQC.TableName = vcPlant + "_" + vcCLYM + "_" + iTimes;
        ...
        public DataTable GetRequestData()
        {
            try
            {
                ...
                sql.Append("	select 'TFTM'+vcPlant as vcPlant,vcCLYM,MAX(iTimes) as iTimes from TNQCStatus        \n");
                sql.Append("	where vcStatus='已请求'        \n");
                sql.Append("	group by vcPlant,vcCLYM       \n");
                sql.Append(")t1    \n");
                sql.Append("inner join (    \n");
                sql.Append("	select vcCLYM,vcPlant,MAX(iTimes) as iTimes from TNQCStatus    \n");
