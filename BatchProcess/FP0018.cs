    ...
    public class FP0018
    {
        ...
        public DataTable SearchNewTPackItem(string strUserId)
        {
            try
            {
                ...
                sql.Append("       from(         \n");
                sql.Append("       select vcChanges,vcPartId,vcReceiver,dFromTime,dToTime,vcCarModel from TSPMaster where isnull(vcDelete,'')<>'1'         \n");
                sql.Append("       )A1 left join         \n");
                sql.Append("    (       \n");
                sql.Append("    --当前有效       \n");
                sql.Append("    select vcpartid,dFromTime as DQYX from dbo.TSPMaster  where isnull(vcDelete,'')<>'1'      \n");
                sql.Append("    and dFromTime<= getdate() and dToTime>=getdate()       \n");
                sql.Append("    )A2 on A1.vcPartId=A2.vcPartId       \n");
                sql.Append("    left join       \n");
                sql.Append("    (       \n");
                sql.Append("    --未来有效       \n");
                sql.Append("    select vcpartid,min(dFromTime) as WLYX from TSPMaster  where isnull(vcDelete,'')<>'1'      \n");
                ...
                sql.Append("       select vcpartid,max(dToTime) as dToTime from TSPMaster   where isnull(vcDelete,'')<>'1'     \n");
                sql.Append("       and dToTime < getdate()        \n");
                sql.Append("       group by vcpartid        \n");
                sql.Append("       ) TC left join       \n");
                sql.Append("       (select vcpartid,dFromTime,dToTime from TSPMaster   where isnull(vcDelete,'')<>'1'     \n");
                ...
                sql.Append("     left join      \n");
                sql.Append("     (      \n");
                sql.Append("      select vcPartId,vcReceiver,min(dFromTime) as dFromTime from TSPMaster   where isnull(vcDelete,'')<>'1'      \n");
                ...
                sql.Append("     left join      \n");
                sql.Append("     (      \n");
                sql.Append("     select vcPartId,vcReceiver,max(dToTime) as dToTime from TSPMaster where isnull(vcDelete,'')<>'1'            \n");
                sql.Append("     group by vcPartId,vcReceiver         \n");
