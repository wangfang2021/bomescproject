    ...
    public class FS0318_DataAccess
    {
        ...
        public DataTable search(string vcCarType, string timeFrom)
        {
            try
            {
                ...
                sbr.AppendLine("(SELECT * FROM VI_SQJD WHERE vcInOutflag = '0' and vcCarType = '" + vcCarType + "' ) a ");
                sbr.AppendLine("");
                sbr.AppendLine("SELECT @outSQALL = COUNT(*)  FROM  ");
                sbr.AppendLine("(SELECT * FROM VI_SQJD WHERE vcInOutflag = '1' and vcCarType = '" + vcCarType + "' ) a ");
                sbr.AppendLine("");
                sbr.AppendLine("SELECT @inSQ = COUNT(*)  FROM  ");
                sbr.AppendLine("(SELECT * FROM VI_SQJD WHERE vcInOutflag = '0' AND vcJD <> '1' and vcCarType = '" + vcCarType + "' ) a ");
                sbr.AppendLine("");
                sbr.AppendLine("SELECT @outSQ = COUNT(*)  FROM  ");
                sbr.AppendLine("(SELECT * FROM VI_SQJD WHERE vcInOutflag = '1' AND vcJD <> '1' and vcCarType = '" + vcCarType + "' ) a ");
                sbr.AppendLine("");
                sbr.AppendLine("DECLARE @inPrice INT ");
                ...
                sbr.AppendLine("(SELECT * FROM dbo.VI_Price WHERE dPricebegin <= @time AND dPriceEnd >=  @time AND vcProjectType = '0' and vcCarTypeDev = '" + vcCarType + "') a ");
                sbr.AppendLine("");
                sbr.AppendLine("SELECT @outPriceALL = COUNT(*)  FROM  ");
                sbr.AppendLine("(SELECT * FROM dbo.VI_Price WHERE dPricebegin <= @time AND dPriceEnd >=  @time AND vcProjectType = '1' and vcCarTypeDev = '" + vcCarType + "') a ");
                sbr.AppendLine("");
                sbr.AppendLine("SELECT @inPrice = COUNT(*)  FROM  ");
                sbr.AppendLine("(SELECT * FROM dbo.VI_Price WHERE dPricebegin <= @time AND dPriceEnd >=  @time AND vcProjectType = '0' AND decPriceOrigin IS NOT NULL and vcCarTypeDev = '" + vcCarType + "') a ");
                sbr.AppendLine("");
                sbr.AppendLine("SELECT @outPrice = COUNT(*)  FROM  ");
                sbr.AppendLine("(SELECT * FROM dbo.VI_Price WHERE dPricebegin <= @time AND dPriceEnd >=  @time AND vcProjectType = '1' AND decPriceOrigin IS NOT NULL and vcCarTypeDev = '" + vcCarType + "') a ");
                sbr.AppendLine("");
                sbr.AppendLine("DECLARE @inHezi INT ");
                ...
                sbr.AppendLine("");
                sbr.AppendLine("SELECT @outHezi = COUNT(*)  FROM  ");
                sbr.AppendLine("(SELECT * FROM dbo.VI_HeZi  WHERE dUseStartDate <= @time AND dUserEndDate >=  @time AND vcIntake IS NOT NULL  and vcCarType = '" + vcCarType + "') a ");
                sbr.AppendLine("");
                sbr.AppendLine("SELECT @outHeziALL = COUNT(*)  FROM  ");
                sbr.AppendLine("(SELECT * FROM dbo.VI_HeZi  WHERE dUseStartDate <= @time AND dUserEndDate >=  @time  and vcCarType = '" + vcCarType + "') a ");
                sbr.AppendLine("");
                sbr.AppendLine("");
                sbr.AppendLine("DECLARE @inPack INT ");
                ...
                sbr.AppendLine("");
                sbr.AppendLine("SELECT @outPack = COUNT(*)  FROM  ");
                sbr.AppendLine("(SELECT * FROM dbo.VI_PackItem WHERE dUsedFrom <= @time AND dUsedTo >= @time AND isnull(vcPackNo,'') <> ''   and vcCar = '" + vcCarType + "') a ");
                sbr.AppendLine("");
                sbr.AppendLine("SELECT @outPackALL = COUNT(*)  FROM  ");
                sbr.AppendLine("(SELECT * FROM dbo.VI_PackItem WHERE dUsedFrom <= @time AND dUsedTo >= @time   and vcCar = '" + vcCarType + "') a ");
                sbr.AppendLine("");
                sbr.AppendLine("SELECT @inSum AS inSum ,@outSum AS outSum,@inSum+@outSum AS total,@inSQ AS inSQ,@outSQ AS outSQ,@inPrice AS inPrice,@outPrice AS outPrice,@inHezi AS inHezi,@outHezi AS outHezi,@inPack AS inPack,@outPack AS outPack, ");
