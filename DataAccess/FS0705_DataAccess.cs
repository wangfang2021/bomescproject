...
namespace DataAccess
{
    public class FS0705_DataAccess
    {
        ...
        public void computer(string strFaZhuID, string strUserID, string strPackSpot,string strRuHeToTime,string strBianCi,string strNaQiDate)
        {
            ...
            strSql.Append("       set @dNaQi = '"+strNaQiDate+"'     \r\n");
            ...
            strSql.Append("              				    (                  \r\n");
            strSql.Append("              				    select  vcPackNo,MAX(iAutoId)as 'iMaxID' from TPackCompute_Ajust where  vcPackSpot = '" + strPackSpot + "'  and ( @dBegin <= dTime and  dTime<=@dEnd) group by vcPackNo                  \r\n");
            strSql.Append("              				    ) a                  \r\n");
            strSql.Append("              				    inner join                  \r\n");
            strSql.Append("              				    (                  \r\n");
            strSql.Append("              				        select iAutoId,vcPackNo,iNumber from  TPackCompute_Ajust where  vcPackSpot = '" + strPackSpot + "' and ( @dBegin <= dTime and  dTime<=@dEnd)                  \r\n");
        ...
        public DataTable getInvalidPackNo(string strFaZhuID,string strPackSpot,string strEnd)
        {
            ...
            strSql.AppendLine("       (      ");
            strSql.AppendLine("       	select vcPart_id,SUM(iQuantity) as 'iQuantity' from TOperateSJ       ");
            ...
            strSql.AppendLine("       left join      ");
            strSql.AppendLine("       (      ");
            strSql.AppendLine("       	select a.vcPartsNo,vcPackNo,vcBZPlant from      ");
            strSql.AppendLine("       	(      ");
            strSql.AppendLine("       		select vcPartsNo,vcPackNo from TPackItem where dUsedFrom<GETDATE() and GETDATE()<dUsedTo      ");
            strSql.AppendLine("       	)a      ");
            strSql.AppendLine("       	left join      ");
            strSql.AppendLine("       	(      ");
            strSql.AppendLine("       		select vcPart_id,vcBZPlant from TPackageMaster      ");
            strSql.AppendLine("       	)b on a.vcPartsNo = b.vcPart_id      ");
            strSql.AppendLine("       	where b.vcBZPlant = '"+strPackSpot+"'      ");
            strSql.AppendLine("       )b on a.vcPart_id = b.vcPartsNo      ");
            strSql.AppendLine("       where vcPartsNo is null or vcPackNo = 'MBC'      ");
            return excute.ExcuteSqlWithSelectToDT(strSql.ToString());
        }
        #endregion
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.AppendLine("       select vcPackNo from TPackBase where vcReleaseName = '" + strFaZhuID + "' and vcPackSpot = '" + strPackSpot + "' and dPackFrom < GETDATE() and dPackTo>GETDATE()      ");
            return excute.ExcuteSqlWithSelectToDT(strSql.ToString());
        }
        #endregion
        ...
        public void AdmitEmptyData(DataTable dt, string strUserID)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    strSql.AppendLine("           ,'XXXXXXX'      ");
                    strSql.AppendLine("       )      ");
                }
                #endregion

                #region 临时表与计算结果表关联，更新计算结果表的订单号
                strSql.AppendLine("        UPDATE TPackCompute set vcOrderNo = b.vcOrderNo,vcOperatorID = '"+strUserID+"'         ");
        ...
        public void SCFZData(DataTable dt, string strOrderNo,string strUserID)
        {
            ...
            try
            {
                 ...
                 * 修改时间：2021-5-7
                 * 修改人：董镇
                 * 修改内容：生成发注数据时，插入发注实际表时，不给发注时间赋值，发注时间应该是取订单发注界面点击发注订单时的时间
                 */
