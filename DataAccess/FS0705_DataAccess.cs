...
namespace DataAccess
{
    public class FS0705_DataAccess
    {
        ...
        public void computer(string strFaZhuID, string strUserID, string strPackSpot)
        {
            ...
            strSql.Append("       if @dBegin is NULL           \r\n");
            strSql.Append("       BEGIN           \r\n");
            strSql.Append("       set @dBegin = (select top 1 dFrom from TPackFaZhuTime where vcFaZhuID = '外注纸箱');     \r\n");
            strSql.Append("       END     \r\n");
            strSql.Append("       IF @dEnd is NULL     \r\n");
            strSql.Append("       BEGIN     \r\n");
            strSql.Append("       set @dEnd = (select  top 1 convert(varchar(10),getdate(),120)+' '+convert(varchar(50),druHeToTime) from TPackFaZhuTime where vcPackSpot = 'H2' and vcFaZhuID = '外注纸箱' and dFaZhuToTime < CONVERT(char(8),GETDATE(),108)   order by dFaZhuToTime DESC);     \r\n");
            strSql.Append("       END     \r\n");
            strSql.Append("       IF @dNaQi is NULL     \r\n");
            strSql.Append("       BEGIN     \r\n");
            strSql.Append("       SET @dNaQi = (select  top 1 convert(varchar(10),getdate(),120)+' '+convert(varchar(50),dNaQiFromTime) from TPackFaZhuTime where vcPackSpot = 'H2' and vcFaZhuID = '外注纸箱' and dFaZhuToTime > CONVERT(char(8),GETDATE(),108)   order by dFaZhuToTime asc)     \r\n");
            strSql.Append("       END     \r\n");
            ...
            strSql.Append("              				    left join                  \r\n");
            strSql.Append("              				    (                  \r\n");
            strSql.Append("              					    select * from TPack_FaZhu_ShiJi where vcPackSpot = 'H2' and dNaRuShiJi is not null and @dBegin<=dNaRuShiJi and dNaRuShiJi<=@dEnd                  \r\n");
            strSql.Append("              				    ) b on a.vcPackNo = b.vcPackNo                  \r\n");
        ...
        public DataTable getPackCheckDT(string strFaZhuID,string strPackSpot) 
        {
            ...
            strSql.Append("       (      \r\n");
            strSql.Append("           select distinct vcPart_id from TOperateSJ where vcBZPlant = 'H2' and vcZYType='S0' and @dBegin<=dEnd and dEnd<=@dEnd      \r\n");
            strSql.Append("       )a       \r\n");
            strSql.Append("       left join       \r\n");
            strSql.Append("       (      \r\n");
            ...
            strSql.Append("          inner join        \r\n");
            strSql.Append("          (      \r\n");
            strSql.Append("              select vcPart_id,vcBZPlant from TPackageMaster where vcBZPlant  = 'H2'      \r\n");
            strSql.Append("          )  b  on a.vcPartsNo  = b.vcPart_id      \r\n");
            strSql.Append("       )b on a.vcPart_id = b.vcPartsNo      \r\n");
