...
{
    public class FS1204_DataAccess
    {
        ...
        public void UpdateTable(DataTable dtrel, string Month)
        {
            ...
            try
            {
                ...
                {
                    int num = 0;
                    string strWeekCount = string.Empty;
                    ...
                    for (int i = 4; i < dtupdate.Columns.Count - 2; i++)//比月度履历多了一列周数，应列数-2
                    {

                        dtupdate.Rows[j][i] = dtrel.Rows[j][i - 2].ToString().Trim();
                        if (Regex.IsMatch(dtrel.Rows[j][i - 2].ToString().Trim(), "(1|2|3|4|5)") && i > 5)//只有是1、2、3、4、5这五个数字是合法的，且累加数值
                        {
                            num++;
                ...
                {
                    dtupdate.Columns["vcPlant"],
                    dtupdate.Columns["vcYear"],
                    dtupdate.Columns["vcMonth"],
                    dtupdate.Columns["vcGC"],
                    dtupdate.Columns["vcZB"]
                };
        ...
        public void UpdateCalendar(string plant, string gc, string zb, string year, string month, string[] data)
        {
            ...
            {
                string strWeekCount = string.Empty;
                string UpTable = "WeekCalendarTbl";
                ...
                {
                    strWeekCount += data[i].Split('~')[0].ToString().Trim();
                }
                SqlCommandBuilder cmdBud = new SqlCommandBuilder(ss);
                ...
                dt.Rows[dt.Rows.Count - 1]["vcTotal"] = data.Length - 1;
                dt.Rows[dt.Rows.Count - 1]["vcWeekCount"] = StringItemCount(strWeekCount).ToString();
                ...
                dt.PrimaryKey = new DataColumn[] { dt.Columns["vcPlant"], dt.Columns["vcYear"], dt.Columns["vcMonth"], dt.Columns["vcGC"], dt.Columns["vcZB"] }; //设置主键
                ss.Update(dt);
                dt.AcceptChanges();
                ss.Dispose();
                cmdBud.Dispose();
        ...
        private static int StringItemCount(string str)
        {
            int num = 0;
            int[] count = new int[5];
            count[0] = Regex.Matches(str, @"1").Count;
            count[1] = Regex.Matches(str, @"2").Count;
            count[2] = Regex.Matches(str, @"3").Count;
            count[3] = Regex.Matches(str, @"4").Count;
            count[4] = Regex.Matches(str, @"5").Count;
