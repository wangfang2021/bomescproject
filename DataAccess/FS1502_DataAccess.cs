    ...
    public class FS1502_DataAccess
    {
        ...
        public void Save(List<Dictionary<string, Object>> listInfoData, string strUserId)
        {
            try
            {
                ...
                for (int i = 0; i < listInfoData.Count; i++)
                {
                    ...
                    string iPlanTZ = listInfoData[i]["iPlanTZ"].ToString()==""?"null": listInfoData[i]["iPlanTZ"].ToString();
                    string iLJBZRemain = listInfoData[i]["iLJBZRemain"].ToString()==""?"null": listInfoData[i]["iLJBZRemain"].ToString();
                    ...
                    {//修改
                        ...
                        sql.Append("update TPackingPlan_Summary set iPlanTZ=" + iPlanTZ + ",iLJBZRemain="+iLJBZRemain+", " +
                            "vcOperatorID='" + strUserId + "',dOperatorTime=getdate() where iAutoId=" + iAutoId + "   \n");
                    }
                }
                if (sql.Length > 0)
                {
        ...
        public void importSave_Sub(DataTable dt,string vcFZPlant,string dBZDate, string strUserId)
        {
            try
            {
                ...
                //统一更新
                sql.Append("update t1 set t1.vcPackBZ=t3.vcValue1     \n");
                //sql.Append("t1.vcBCPartsNo=t4.vcPackNo,t1.vcSmallPM=t5.vcSmallPM,t1.vcBigPM=t6.vcBigPM    \n");
                sql.Append("from (    \n");
