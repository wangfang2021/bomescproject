    ...
    public class FS1502_DataAccess
    {
        ...
        public void importSave_Sub(DataTable dt, string vcFZPlant, string dBZDate, string strUserId, string strUnit)
        {
            ...
            try
            {
                ...
                sql.AppendLine("from");
                sql.AppendLine("(");
                sql.AppendLine("	select * from TPackingPlan where vcFZPlant='" + vcFZPlant + "' and vcPackDate='" + dBZDate + "' and vcType='N'  ");
                sql.AppendLine(")t1");
                ...
                //更新N日的班值
                sql.AppendLine("update t1 set t1.vcPackBZ=case when t3.vcCodeId is not null then t3.vcValue1 else t4.vcValue1 end    ");
                sql.AppendLine("from    ");
                sql.AppendLine("(select * from TPackingPlan where vcFZPlant='" + vcFZPlant + "' and vcPackDate='" + dBZDate + "' and vcType='N'   ");
                sql.Append(")t1    ");
                sql.AppendLine("inner join (");
                ...
                sql.AppendLine("(    ");
                sql.AppendLine("	select vcCodeId,vcCodeName,vcValue1,    ");
                sql.AppendLine("	  case when vcValue1='白' then cast('2001-01-01 '+vcValue2 as datetime)     ");
                sql.AppendLine("			when vcValue1='夜' then cast('2001-01-01 '+vcValue2 as datetime)    ");
                sql.AppendLine("	   end as vcValue2 ,    ");
                sql.AppendLine("       case when vcValue1='白' then cast('2001-01-01 '+vcValue3 as datetime)     ");
                sql.AppendLine("	        when vcValue1='夜' then cast('2001-01-02 '+vcValue3 as datetime)    ");
                sql.AppendLine("	   end as vcValue3     ");
                sql.AppendLine("	from TOutCode where vcCodeId='C002' and vcIsColum='0'    ");
                sql.AppendLine(")t3 on cast('2001-01-01 '+t2.vcNRBJSK as datetime) between cast(t3.vcValue2 as datetime) and cast(t3.vcValue3 as datetime)    ");
                sql.AppendLine("left join    ");
                sql.AppendLine("(    ");
                sql.AppendLine("	select vcCodeId,vcCodeName,vcValue1,    ");
                sql.AppendLine("	  case when vcValue1='白' then cast('2001-01-01 '+vcValue2 as datetime)     ");
                ...
                sql.AppendLine("	   end as vcValue3     ");
                sql.AppendLine("	from TOutCode where vcCodeId='C002' and vcIsColum='0'    ");
                sql.AppendLine(")t4 on cast('2001-01-02 '+t2.vcNRBJSK as datetime) between cast(t4.vcValue2 as datetime) and cast(t4.vcValue3 as datetime)    ");
                ...
                sql.Append("		left join TPMRelation t3 on t2.vcSmallPM=t3.vcSmallPM    \n");
                sql.Append("        left join (select * from TSPMaster where vcReceiver='APC06' and vcPackingPlant='" + strUnit + "' and dFromTime<>dToTime and ISNULL(vcDelete,'')<>'1' )t4          \n");
