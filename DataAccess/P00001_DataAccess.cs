  ...
  public class P00001_DataAccess
  {
    ...
    public DataSet getCheckQBandSJInfo(string partId, string kanbanOrderNo, string kanbanSerial, string dock, string packingSpot, string scanTime, string strType)
    {
      ...
      if (strType == "QB")
      {
        ...
        stringBuilder.AppendLine("select  vcBZQF, vcRHQF, vcBZUnit from TPackageMaster where vcPart_id = '" + partId + "' and dTimeFrom <= convert(varchar(10),GETDATE(),23)  and dTimeTo >=convert(varchar(10),GETDATE(),23)  and vcBZQF is not null and vcRHQF is not null and vcBZUnit is not null");
        stringBuilder.AppendLine("--2.检验受入");
        stringBuilder.AppendLine("select vcReceiver, vcPackingPlant from TSPMaster_SufferIn where vcPackingPlant = 'TFTM' and vcPartId = '" + partId + "' and  dFromTime <= convert(varchar(10),GETDATE(),23)  and dToTime >= convert(varchar(10),GETDATE(),23)  and vcSufferIn = '" + dock + "'");
        stringBuilder.AppendLine("--3.检验收容数");
        stringBuilder.AppendLine("select iPackingQty,vcSupplierId,vcSupplierPlant from TSPMaster_Box where vcPartId='" + partId + "' and dFromTime<=convert(varchar(10),GETDATE(),23)  and dToTime>=convert(varchar(10),GETDATE(),23) ");
        stringBuilder.AppendLine("--4.检验内外");
        stringBuilder.AppendLine("select vcInOut,vcPartENName,vcCarfamilyCode,vcSupplierName,vcSupplierPlace from TSPMaster WHERE vcPartId='" + partId + "' and dFromTime<=convert(varchar(10),GETDATE(),23) and dToTime>=convert(varchar(10),GETDATE(),23) ");
        stringBuilder.AppendLine("--5.包材");
        stringBuilder.AppendLine("select a.vcPackNo,a.iBiYao,b.vcPackLocation,b.vcDistinguish from ");
        stringBuilder.AppendLine("(select vcPackNo,iBiYao from TPackItem where vcPartsNo='" + partId + "' and dUsedFrom<=convert(varchar(10),GETDATE(),23)  and dUsedTo>=convert(varchar(10),GETDATE(),23) )a");
        stringBuilder.AppendLine("left join");
        stringBuilder.AppendLine("(select vcPackLocation,vcDistinguish,vcPackNo from TPackBase where vcPackSpot='" + packingSpot + "' and dPackFrom<=convert(varchar(10),GETDATE(),23)  and dPackTo>=convert(varchar(10),GETDATE(),23) )b");
        stringBuilder.AppendLine("on a.vcPackNo=b.vcPackNo");
        stringBuilder.AppendLine("--6.标签");
        stringBuilder.AppendLine("select vcPartNameCN, vcSCSName, vcSCSAdress, vcZXBZNo from TtagMaster where vcPart_Id = '" + partId + "' and dTimeFrom <= convert(varchar(10),GETDATE(),23)  and dTimeTo >= convert(varchar(10),GETDATE(),23) ");
        ...
        stringBuilder.AppendLine("select b.decPriceTNPWithTax from");
        stringBuilder.AppendLine("(select vcPartId,vcSupplierId,vcReceiver from TSPMaster where vcPartId='" + partId + "' )a left join");
        stringBuilder.AppendLine("(select vcPart_id,vcSupplier_id,vcReceiver,decPriceTNPWithTax from TPrice where dPricebegin<=convert(varchar(10),GETDATE(),23)  and dPriceEnd>=convert(varchar(10),GETDATE(),23) and dPricebegin<>dPriceEnd ) b");
        stringBuilder.AppendLine("on a.vcPartId=b.vcPart_id and a.vcSupplierId=b.vcSupplier_id and a.vcReceiver=b.vcReceiver");
        stringBuilder.AppendLine("where isnull(b.decPriceTNPWithTax,0) <>0");
        ...
        stringBuilder.AppendLine("(select * from TSPMaster where vcPartId='" + partId + "' and dFromTime<=convert(varchar(10),GETDATE(),23) and dToTime>=convert(varchar(10),GETDATE(),23))a");
        stringBuilder.AppendLine("left join");
      ...
      if (strType == "SJ")
      {
        ...
        stringBuilder.AppendLine("select vcInOut,vcPartENName,vcCarfamilyCode,vcSupplierName,vcSupplierPlace	 from TSPMaster WHERE vcPartId='" + partId + "' and dFromTime<=convert(varchar(10),GETDATE(),23) and dToTime>=convert(varchar(10),GETDATE(),23)");
        stringBuilder.AppendLine("--5.包材");
        stringBuilder.AppendLine("select vcPackNo,iBiYao from TPackItem where vcPartsNo='" + partId + "' and dUsedFrom<=convert(varchar(10),GETDATE(),23) and dUsedTo>=convert(varchar(10),GETDATE(),23) and  dFrom<=convert(varchar(10),GETDATE(),23) and  dTo>=convert(varchar(10),GETDATE(),23)");
        stringBuilder.AppendLine("--6.标签");
        stringBuilder.AppendLine("select vcPartNameCN, vcSCSName, vcSCSAdress, vcZXBZNo from TtagMaster where vcPart_Id = '" + partId + "' and dTimeFrom <= convert(varchar(10),GETDATE(),23) and dTimeTo >= convert(varchar(10),GETDATE(),23)");
        stringBuilder.AppendLine("--7.订单量");
        stringBuilder.AppendLine("select (ISNULL(SUM(CAST(vcPlantQtyDailySum as int)),0)-ISNULL(SUM(CAST(vcInputQtyDailySum as int)),0)) as sum from SP_M_ORD where ISNULL(vcPlantQtyDailySum,0)!=ISNULL(vcInputQtyDailySum,0) and vcPartNo='" + partId + "' and vcOrderNo!=''");
    ...
    public DataSet getInputInfoFromDB(string strIP, string serverTime)
    {
      ...
      stringBuilder.AppendLine("left join");
      stringBuilder.AppendLine("(select * from TPackageMaster where dTimeFrom<=convert(varchar(10),GETDATE(),23) and dTimeTo>=convert(varchar(10),GETDATE(),23) )t2");
      stringBuilder.AppendLine("on t1.vcPart_id=t2.vcPart_id  ");
      stringBuilder.AppendLine("left join ");
      stringBuilder.AppendLine("(select vcPackingPlant,vcPartId,vcSupplierId,vcReceiver,vcInOut,vcPartENName,vcCarfamilyCode,vcSupplierName,vcSupplierPlace from TSPMaster WHERE dFromTime<=convert(varchar(10),GETDATE(),23) and dToTime>=convert(varchar(10),GETDATE(),23))b ");
      ...
      stringBuilder.AppendLine("SELECT TOP (1)*,'' as [vcLabelCode]  FROM [TLabelList]");
      ...
      stringBuilder.AppendLine("--11用于入荷消订单");
      stringBuilder.AppendLine("select vcInputNo,vcPart_id,vcKBOrderNo,vcKBLFNo,vcSR,vcCpdCompany,iQuantity,iQuantity as iSumQuantity from TOperatorQB");
      stringBuilder.AppendLine("where vcHostIp='" + strIP + "' and vcZYType='S0' and vcReflectFlag='0'");
      stringBuilder.AppendLine("order by vcPart_id");
      stringBuilder.AppendLine("--12用于入荷消订单");
      stringBuilder.AppendLine("SELECT top(1)*  FROM [SP_M_ORD_INOUT]");
    ...
    public DataTable getPackInfo(string strIP, string serverTime)
    {
      ...
      stringBuilder.AppendLine("		,d.vcBZQF as vcBZQF");
      stringBuilder.AppendLine("		from ");
      stringBuilder.AppendLine("(select * from TOperatorQB  where vcHostIp='" + strIP + "' and vcZYType='S0' and vcReflectFlag='0')a");
      stringBuilder.AppendLine("left join");
      stringBuilder.AppendLine("(select *  from TPackItem where dUsedFrom<=convert(varchar(10),GETDATE(),23) and dUsedTo>=convert(varchar(10),GETDATE(),23))b");
      stringBuilder.AppendLine("on a.vcPart_id = b.vcPartsNo and b.dFrom<=convert(varchar(10),GETDATE(),23)and b.dTo>=convert(varchar(10),GETDATE(),23)");
      stringBuilder.AppendLine("left join");
      stringBuilder.AppendLine("(select vcPackLocation,vcDistinguish,vcPackSpot,vcPackNo from TPackBase where dPackFrom<=convert(varchar(10),GETDATE(),23) and dPackTo>=convert(varchar(10),GETDATE(),23))c");
      stringBuilder.AppendLine("on b.vcPackNo=c.vcPackNo and a.vcBZPlant=c.vcPackSpot");
      stringBuilder.AppendLine("left join");
      stringBuilder.AppendLine("(select * from TPackageMaster where dTimeFrom<=convert(varchar(10),GETDATE(),23) and dTimeTo>=convert(varchar(10),GETDATE(),23))d");
    ...
    public DataTable getLabelInfo(string strIP, string serverTime)
    {
      ...
      stringBuilder.AppendLine("left join");
      stringBuilder.AppendLine("(select * from TPackageMaster where dTimeFrom<=convert(varchar(10),GETDATE(),23) and dTimeTo>=convert(varchar(10),GETDATE(),23))c");
      stringBuilder.AppendLine("on a.vcPart_id=c.vcPart_id");
      stringBuilder.AppendLine("left join");
      stringBuilder.AppendLine("(select vcPart_Id,vcCPDCompany,vcSupplier_id,vcPartNameCN,vcCarTypeName,vcSCSName,vcSCSAdress,vcZXBZNo from  TtagMaster where dTimeFrom<=convert(varchar(10),GETDATE(),23) and dTimeTo>=convert(varchar(10),GETDATE(),23))d");
    ...
    public DataTable getOrderInfo(string strIP, string serverTime)
    {
      ...
      stringBuilder.AppendLine("SELECT ROW_NUMBER() OVER(ORDER BY [vcPartNo],[vcTargetYearMonth],[iSequence],[vcOrderNo],[vcSeqno],iAutoId)-1 AS iRows");
      stringBuilder.AppendLine("	  ,[vcTargetYearMonth],[vcOrderNo],[vcSeqno],[vcOrderDifferentiation],[vcOrderInitials],[iSequence]");
      stringBuilder.AppendLine("    ,[iAutoId],[vcPartNo],[vcDock],b.vcCpdcompany,a.iSumQuantity");
      stringBuilder.AppendLine("    ,[day1],[day2],[day3],[day4],[day5],[day6],[day7],[day8],[day9],[day10]");
      stringBuilder.AppendLine("    ,[day11],[day12],[day13],[day14],[day15],[day16],[day17],[day18],[day19],[day20]");
      stringBuilder.AppendLine("	  ,[day21],[day22],[day23],[day24],[day25],[day26],[day27],[day28],[day29],[day30],[day31] from");
      stringBuilder.AppendLine("(select vcPart_id,vcCpdCompany,vcSR,sum(iQuantity) as iSumQuantity from TOperatorQB  ");
      stringBuilder.AppendLine("where vcHostIp='" + strIP + "' and vcZYType='S0' and vcReflectFlag='0'");
      stringBuilder.AppendLine("group by vcPart_id,vcCpdCompany,vcSR)a");
      stringBuilder.AppendLine("left join");
      stringBuilder.AppendLine("(SELECT *  FROM [V_SP01_INORD])b");
    ...
    public bool setInputInfo(string strIP, string strPointName, string strPrinterName, DataTable dtPackList_Temp, DataTable dtLabelList_Temp, DataTable dtInv_Temp, DataTable dtOrder_Temp, DataTable dtORD_INOUT_Temp, string strOperId, string strPackPrinterName)
    {
      ...
      try
      {
        ...
        strSql_addinfo.AppendLine("update TPackList set dFirstPrintTime=GETDATE() where dFirstPrintTime is null and vcHostip='" + strIP + "'");
        strSql_addinfo.AppendLine("update TPackList_Temp set dFirstPrintTime=GETDATE() where dFirstPrintTime is null and vcHostip='" + strIP + "'");
        ...
        strSql_modinfo_sp_ord_inout.AppendLine("           ,@vcKBLFNo");
        strSql_modinfo_sp_ord_inout.AppendLine("           ,@vcSR");
        strSql_modinfo_sp_ord_inout.AppendLine("           ,'IN'");
        strSql_modinfo_sp_ord_inout.AppendLine("           ,@iQuantity");
        strSql_modinfo_sp_ord_inout.AppendLine("           ,@vcOrderNo");
        strSql_modinfo_sp_ord_inout.AppendLine("           ,@vcSeqno");
        strSql_modinfo_sp_ord_inout.AppendLine("           ,@iDay");
