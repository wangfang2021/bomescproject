...
namespace DataAccess
{
  public class P00001_DataAccess
  {
    ...
    public DataSet CheckDb(string partId, string kanbanOrderNo, string kanbanSerial, string dock, string scanTime)
    {
      ...
      CheckDbSql.AppendLine("select vcPart_id from TOperatorQB where vcReflectFlag in ('0','1') and vcKBOrderNo='" + kanbanOrderNo + "' and vcKBLFNo='" + kanbanSerial + "' and vcPart_id='" + partId + "' and vcSR='" + dock + "' and vcZYType='S0'");
      ...
      CheckDbSql.AppendLine("select b.decPriceOrigin_CW from");
      CheckDbSql.AppendLine("(select vcPartId,vcSupplierId,vcReceiver from TSPMaster where vcPartId='"+partId+"' )a left join");
      CheckDbSql.AppendLine("(select vcPart_id,vcSupplier_id,vcReceiver,decPriceOrigin_CW from TPrice where dPricebegin<='"+scanTime+"' and dPriceEnd>='"+scanTime+"') b");
      CheckDbSql.AppendLine("on a.vcPartId=b.vcPart_id and a.vcSupplierId=b.vcSupplier_id and a.vcReceiver=b.vcReceiver");
    ...
    public void Excute(string sql)
    {
      ...
      try
      {
        ...
        sqlCommand_sub.CommandText = sql;
    ...
    public DataSet CheckDb1(string partId, string kanbanOrderNo, string kanbanSerial, string dock, object scanTime)
    {
      StringBuilder CheckDbSql = new StringBuilder();
    ...
    public DataTable GetID(string iP, string serverTime)
    {
      ...
      GetIDSql.Append("where t1.vcPart_id=t2.vcPart_id and t2.dTimeFrom<='" + serverTime + "' and t2.dTimeTo>='" + serverTime + "' and \n");
    ...
    public string UpdatePack(string iP, string minLabel, string maxLabel, string lotIdNew)
    {
      StringBuilder UpdatePackSql = new StringBuilder();
    ...
    public string InsertTP1(string iP, string opearteId, string serverTime, string printName)
    {
    ...
    */
    ...
    public string InsertTP(string iP, string opearteId, string serverTime, string lotIdNew)
    {
      StringBuilder insertTPSql = new StringBuilder();
      insertTPSql.Append("INSERT INTO TPrint_Temp(vcTableName,vcReportName,vcClientIP,vcPrintName,vcKind,vcOperatorID,dOperatorTime,vcCaseNo,vcSellNo,vcLotid,vcSupplierId, vcInno, vcFlag)");
    ...
    public DataTable getPackInfo(string strIP, string serverTime)
    {
      ...
      stringBuilder.AppendLine("left join");
      stringBuilder.AppendLine("(select * from TPackageMaster where dTimeFrom<='" + serverTime + "' and dTimeTo>='" + serverTime + "' )d");
