...
namespace DataAccess
{
  public class P00003_DataAccess
  {
    ...
    public DataTable GetDBZ(string partId, string kanbanOrderNo, string kanbanSerial, string dock)
    {
      StringBuilder GetDataSql = new StringBuilder();
      GetDataSql.Append("  select iDBZ from TOperateSJ_InOutput where vcPart_id='" + partId + "' and vcKBOrderNo='" + kanbanOrderNo + "' and vcKBLFNo='" + kanbanSerial + "' and vcSR='" + dock + "'");
      SqlConnection ConnSql = Common.ComConnectionHelper.CreateSqlConnection();
