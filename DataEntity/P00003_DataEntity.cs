using System.Collections.Generic;
using System.Data;

namespace DataEntity
{
  public class P00003_DataEntity
  {
    public string NZ { get; set; }

    public string GZ { get; set; }

    public string caseNo { get; set; }
    public string boxNo { get; set; }
        public string caseNo1 { get; set; }
    public string boxNo1 { get; set; }
    public string caseNo2 { get; set; }
    public string boxNo2 { get; set; }
    public string caseNo3 { get; set; }
    public string boxNo3 { get; set; }

    public string packTotalTime { get; set; }






    public string other { get; set; }

    public string PM { get; set; }

    public string DW { get; set; }
    public string quantity { get; set; }
    public string checkType { get; set; }

    public string kanbanQuantity { get; set; }

    public string packQuantity { get; set; }


    public string caseQuantity { get; set; }

    public DataTable packParts { get; set; }

    public string packingQuatity { get; set; }//包装单位

    public string totalTime { get; set; }


    public string freQuency { get; set; }


    public string effiEncy { get; set; }

    public string startTime { get; set; }



    public string userName { get; set; }

    public string partId { get; set; }

    public string dock { get; set; }

    public string kanbanOrderNo { get; set; }

    public string kanbanSerial { get; set; }

    public string kanban { get; set; }

    public string photo { get; set; }


    public DataTable caseList { get; set; }


    public string result { get; set; }


    public string banZhi { get; set; }

    public string stanTime { get; set; }

    public string active { get; set; }


  }
}
