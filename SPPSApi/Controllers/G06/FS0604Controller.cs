...
namespace SPPSApi.Controllers.G06
{
    ...
    public class FS0604Controller : BaseController
    {
        ...
        public string changeSupplieridApi([FromBody] dynamic data)
        {
            ...
            {
                ComMessage.GetInstance().ProcessMessage(FunctionID, "M06UE0418", ex, loginInfo.UserId);
                apiResult.code = ComConstant.ERROR_CODE;
        ...
        public string saveApi([FromBody] dynamic data)
        {
            ...
            try
            {
                ...
                if (hasFind)
                {
                    string[,] strField = new string[,] {{"状态","展开时间","要望纳期","同步时间","包装工场", "收货方",  "品番","品名", "车型", "使用开始时间",  "OE=SP", "供应商代码", "工区",  "要望收容数", "收容数", "箱最大收容数", "箱种", "长(mm)", "宽(mm)", "高(mm)", "空箱重量(g)", "单品净重(g)", "照片","回复时间","承认时间","原单位织入时间","备注"},
        ...
        public string deleteApi([FromBody] dynamic data)
        {
            ...
            {
                ComMessage.GetInstance().ProcessMessage(FunctionID, "M06UE0411", ex, loginInfo.UserId);
                apiResult.code = ComConstant.ERROR_CODE;
                apiResult.data = "删除失败";
                return JsonConvert.SerializeObject(apiResult, Formatting.Indented, JSON_SETTING);
            }
        ...
        public string getDataApi([FromBody] dynamic data)
        {
            ...
            try
            {
                fs0604_Logic.getData(loginInfo.UserId);
                apiResult.code = ComConstant.SUCCESS_CODE;
                apiResult.data = null;
                return JsonConvert.SerializeObject(apiResult, Formatting.Indented, JSON_SETTING);
            }
            ...
            {
                ComMessage.GetInstance().ProcessMessage(FunctionID, "M06UE0413", ex, loginInfo.UserId);
                apiResult.code = ComConstant.ERROR_CODE;
                apiResult.data = "同步数据失败";
                return JsonConvert.SerializeObject(apiResult, Formatting.Indented, JSON_SETTING);
            }
        }
        #endregion
