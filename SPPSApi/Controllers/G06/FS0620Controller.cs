...
namespace SPPSApi.Controllers.G06
{
    ...
    public class FS0620Controller : BaseController
    {
        ...
        public string changeSupplieridApi([FromBody] dynamic data)
        {
            ...
            {
                ComMessage.GetInstance().ProcessMessage(FunctionID, "M06UE2012", ex, loginInfo.UserId);
                apiResult.code = ComConstant.ERROR_CODE;
        ...
        public string exportmessageApi([FromBody] dynamic data)
        {
            ...
            {
                ComMessage.GetInstance().ProcessMessage(FunctionID, "M06UE2006", ex, loginInfo.UserId);
                apiResult.code = ComConstant.ERROR_CODE;
                apiResult.data = "导出消息失败";
                return JsonConvert.SerializeObject(apiResult, Formatting.Indented, JSON_SETTING);
            }
        }
        ...
        public string createReportApi([FromBody] dynamic data)
        {
            ...
            {
                ComMessage.GetInstance().ProcessMessage(FunctionID, "M06UE2007", ex, loginInfo.UserId);
                apiResult.code = ComConstant.ERROR_CODE;
        ...
        [HttpPost]
        [EnableCors("any")]
        public string exportmessageImportApi([FromBody] dynamic data)
        {
            //验证是否登录
            string strToken = Request.Headers["X-Token"];
            if (!isLogin(strToken))
            ...
            try
            {
                ...
                string filepath = ComFunction.generateExcelWithXlt(dataTable, fields, _webHostEnvironment.ContentRootPath, "FS0620_Impot_MessageList.xlsx", 1, loginInfo.UserId, FunctionID);
                if (filepath == "")
                {
                    apiResult.code = ComConstant.ERROR_CODE;
                    apiResult.data = "导出生成文件失败";
