    ...
    public class FS0626Controller : BaseController
    {
        ...
        public string exportApi([FromBody] dynamic data)
        {
            ...
            try
            {
                ...
                string filepath = ComFunction.generateExcelWithXlt(dt, fields, _webHostEnvironment.ContentRootPath, "FS0626.xlsx", 1, loginInfo.UserId, FunctionID, true);
                if (filepath == "")
                {
                    apiResult.code = ComConstant.ERROR_CODE;
        ...
        public string updateDataApi([FromBody] dynamic data)
        {
            ...
            ApiResult apiResult = new ApiResult();
            dynamic dataForm = JsonConvert.DeserializeObject(Convert.ToString(data));
            string vcTargetMonthFrom = dataForm.vcTargetMonthFrom == null ? "" : dataForm.vcTargetMonthFrom;
            string vcTargetMonthTo = dataForm.vcTargetMonthTo == null ? "" : dataForm.vcTargetMonthTo;
            try
            {
                bool b = fs0626_Logic.updateData(vcTargetMonthFrom.Replace("-", ""), vcTargetMonthTo.Replace("-", ""), loginInfo.UserId);
                if (!b)
                {
                    apiResult.code = ComConstant.ERROR_CODE;
                    apiResult.data = "更新失败！";
