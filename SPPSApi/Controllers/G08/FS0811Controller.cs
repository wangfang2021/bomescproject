...
namespace SPPSApi.Controllers.G08
{
    ...
    public class FS0811Controller : BaseController
    {
        ...
        public string pageloadApi()
        {
            ...
            {
                ComMessage.GetInstance().ProcessMessage(FunctionID, "M08PE1100", ex, loginInfo.UserId);
                apiResult.code = ComConstant.ERROR_CODE;
        ...
        public string selectApi([FromBody]dynamic data)
        {
            ...
            {
                ComMessage.GetInstance().ProcessMessage(FunctionID, "M08PE1101", ex, loginInfo.UserId);
                apiResult.code = ComConstant.ERROR_CODE;
        ...
        public string searchApi([FromBody]dynamic data)
        {
            ...
            {
                ComMessage.GetInstance().ProcessMessage(FunctionID, "M08PE1102", ex, loginInfo.UserId);
                apiResult.code = ComConstant.ERROR_CODE;
                apiResult.data = "检索失败";
                return JsonConvert.SerializeObject(apiResult, Formatting.Indented, JSON_SETTING);
            }
        ...
        public string queryApi([FromBody]dynamic data)
        {
            ...
            {
                ComMessage.GetInstance().ProcessMessage(FunctionID, "M08PE1103", ex, loginInfo.UserId);
                apiResult.code = ComConstant.ERROR_CODE;
                apiResult.data = "计算失败";
                return JsonConvert.SerializeObject(apiResult, Formatting.Indented, JSON_SETTING);
            }
        ...
        public string saveApi([FromBody]dynamic data)
        {
            ...
            {
                ComMessage.GetInstance().ProcessMessage(FunctionID, "M08PE1104", ex, loginInfo.UserId);
                apiResult.code = ComConstant.ERROR_CODE;
                apiResult.data = "保存失败";
                return JsonConvert.SerializeObject(apiResult, Formatting.Indented, JSON_SETTING);
            }
        ...
        [HttpPost]
        [EnableCors("any")]
        public string dailyPaperApi([FromBody]dynamic data)
        {
            //验证是否登录
            string strToken = Request.Headers["X-Token"];
            if (!isLogin(strToken))
        ...
        [HttpPost]
        [EnableCors("any")]
        public string dailyInfoApi([FromBody]dynamic data)
        {
            //验证是否登录
            string strToken = Request.Headers["X-Token"];
            if (!isLogin(strToken))
        ...
        [HttpPost]
        [EnableCors("any")]
        public string monthPaperApi([FromBody]dynamic data)
        {
            //验证是否登录
            string strToken = Request.Headers["X-Token"];
            if (!isLogin(strToken))
