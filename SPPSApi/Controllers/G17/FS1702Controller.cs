        ...
        public string qrdPrintApi([FromBody]dynamic data)
        {
            ...
            try
            {
                ...
                if (listInfoData.Count != 0)
                {
                    ...
                    //    var callClient = factory.CreateChannel();
                    //    setCRVPrintRequestBody Body = new setCRVPrintRequestBody();
                    //    Body.strScrpit = "select cast(ROW_NUMBER() over(order by UUID,iQuantity) as int) as id,vcOperator,dOperatorTime,vcQueRenNo,vcPart_id,vcBackPart_id,iQuantity from tPrintTemp_FS1702 where vcOperator='" + loginInfo.UserId + "' and iQuantity>0 ORDER BY UUID";
